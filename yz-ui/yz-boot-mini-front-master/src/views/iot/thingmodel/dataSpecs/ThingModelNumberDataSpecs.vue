...
const validateMin = (_: any, __: any, callback: any) => {
  const minValue = dataSpecs.value.min
  if (isEmptyValue(minValue)) {
    callback()
    return
  }
  const min = Number(minValue)
  const maxValue = dataSpecs.value.max
  const max = isEmptyValue(maxValue) ? NaN : Number(maxValue)
  if (isNaN(min)) {
...
const validateMax = (_: any, __: any, callback: any) => {
  ...
  const minValue = dataSpecs.value.min
  const min = isEmptyValue(minValue) ? NaN : Number(minValue)
...
const validateStep = (_: any, __: any, callback: any) => {
  const stepValue = dataSpecs.value.step
  if (isEmptyValue(stepValue)) {
    callback()
    return
  }
