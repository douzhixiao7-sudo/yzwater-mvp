  ...
  <div class="buffer-page">
    ...
    <el-dialog v-model="detailVisible" width="980px" :destroy-on-close="true" align-center>
      ...
      <div v-loading="detailLoading" class="buffer-page__detail-body">
        ...
        <div class="buffer-page__detail-kpi-row">
          <span>涉及行政区</span>
          <span>{{ detailAdminAreaText }}</span>
        </div>
        <div class="buffer-page__detail-kpi-row">
          <span>查询类型</span>
          <span>{{ formatFacilityTypes(detailData?.facilityTypes || []) }}</span>
        </div>
      </div>
...
defineOptions({ name: 'YzBufferQuery' })
...
const detailAdminAreaText = computed(() => {
  const list = (detailData.value?.adminAreas || []) as Array<{ name?: string }>
