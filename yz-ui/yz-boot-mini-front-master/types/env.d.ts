...
interface ImportMetaEnv {
  ...
  /** 退出登录后跳转的导航页完整 URL（若设置则优先于内网/外网自动判断） */
  readonly VITE_LOGOUT_NAVIGATION_URL?: string
  /** 退出后跳转导航页（当前 host 为 192.168.6.101* 时，可选覆盖默认内网地址） */
  readonly VITE_LOGOUT_NAVIGATION_URL_INTRANET?: string
  /** 退出后跳转导航页（非上述 host 时，可选覆盖默认外网地址） */
  readonly VITE_LOGOUT_NAVIGATION_URL_EXTRANET?: string
  // API 加解密相关配置
