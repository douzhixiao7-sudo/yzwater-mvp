...
public class SignboardBfService {
    ...
    public PageResult<SignboardPageRespVO> getSignboardPage(SignboardPageReqVO reqVO) {
        LambdaQueryWrapper<YzSignboardBfDO> wrapper = signboardMapper.buildQueryWrapper(reqVO);
        PageResult<YzSignboardBfDO> page = signboardMapper.selectPage(reqVO, wrapper);
    ...
    public SignboardSaveReqVO getSignboardDetail(Long id) {
        ...
        YzSignboardBfDO signboard = getSignboardById(id);
        if (signboard == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.YZ_SIGNBOARD_NOT_EXISTS);
        }
    ...
    public Long createSignboard(SignboardSaveReqVO reqVO) {
        ...
        YzSignboardBfDO signboard = new YzSignboardBfDO();
    ...
    public void updateSignboard(SignboardSaveReqVO reqVO) {
        ...
        validateExclusiveRelation(reqVO);
        LocalDateTime now = LocalDateTime.now();
        YzSignboardBfDO signboard = getSignboardById(reqVO.getId());
        if (signboard == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.YZ_SIGNBOARD_NOT_EXISTS);
        }
        signboard.setSignboardCode(reqVO.getSignboardCode());
        ...
        signboardMapper.update(null, new LambdaUpdateWrapper<YzSignboardBfDO>()
                .eq(YzSignboardBfDO::getId, signboard.getId())
                .set(YzSignboardBfDO::getIsScreenDisplay, isScreenDisplay)
                .set(YzSignboardBfDO::getUpdateTime, now));
     ...
     */
    private YzSignboardBfDO getSignboardById(Long id) {
        if (id == null) {
            return null;
        }
        return signboardMapper.selectFirstOne(YzSignboardBfDO::getId, id);
    }
    ...
    public List<SignboardExportExcelVO> getSignboardExportList(SignboardPageReqVO reqVO) {
        LambdaQueryWrapper<YzSignboardBfDO> wrapper = signboardMapper.buildQueryWrapper(reqVO);
        List<YzSignboardBfDO> list = signboardMapper.selectList(wrapper);
        Map<String, String> whdwMap = loadDictLabelMap(ZdConstants.ZD_WHDW);
        Map<String, String> gldwMap = loadDictLabelMap(ZdConstants.ZD_GLDW);
        Map<String, String> qsdwMap = loadDictLabelMap(ZdConstants.ZD_QSDW);
        return list.stream()
    ...
    private void fillRiverRelation(YzSignboardBfDO signboard, Long riverChannelId, Long riverSectionId) {
        ...
        if (riverSectionId != null) {
            YzRiverSectionBfDO section = riverSectionMapper.selectById(riverSectionId);
            if (section == null) {
            ...
            YzRiverChannelBfDO channel = channelId == null ? null : riverChannelMapper.selectById(channelId);
            if (channel == null) {
        ...
        YzRiverChannelBfDO channel = riverChannelMapper.selectById(riverChannelId);
        if (channel == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.YZ_RIVER_CHANNEL_NOT_EXISTS);
        }
        signboard.setRiverChannelId(channel.getId());
    ...
    private void fillReservoirRelation(YzSignboardBfDO signboard, Long waterReservoirId) {
        if (waterReservoirId == null) {
            signboard.setWaterReservoirId(null);
            return;
        }
        YzWaterReservoirBfDO reservoir = waterReservoirMapper.selectById(waterReservoirId);
        if (reservoir == null) {
    ...
    private void fillReferenceRelation(YzSignboardBfDO signboard) {
        if (signboard == null) {
            return;
        }
    ...
    private String generateUniqueQrCode() {
        for (int attempt = 0; attempt < 10; attempt++) {
            ...
            YzSignboardBfDO exists = signboardMapper.selectOne(YzSignboardBfDO::getQrCode, qrCode);
            if (exists == null) {
                return qrCode;
            }
        }
    ...
    private SignboardPageRespVO buildPageResp(YzSignboardBfDO item,
    ...
    private Map<Long, String> loadReservoirNameMap(List<YzSignboardBfDO> signboards) {
        ...
        List<Long> reservoirIds = signboards.stream()
                .map(YzSignboardBfDO::getWaterReservoirId)
        ...
        List<YzWaterReservoirBfDO> reservoirs = waterReservoirMapper.selectBatchIds(reservoirIds);
        if (reservoirs == null || reservoirs.isEmpty()) {
            return Map.of();
        }
        return reservoirs.stream()
                .filter(Objects::nonNull)
                .collect(Collectors.toMap(YzWaterReservoirBfDO::getId,
                        item -> StrUtil.blankToDefault(item.getReservoirName(), ""),
                        (left, right) -> left));
    }

    private SignboardExportExcelVO buildExportVO(YzSignboardBfDO item,
                                                 Map<String, String> maintenanceUnitMap,
                                                 Map<String, String> managementUnitMap,
                                                 Map<String, String> ownershipUnitMap) {
