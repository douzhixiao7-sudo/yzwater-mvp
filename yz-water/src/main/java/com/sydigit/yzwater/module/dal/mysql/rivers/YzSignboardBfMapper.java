 ...
 */
@Mapper
public interface YzSignboardBfMapper extends BaseMapperX<YzSignboardBfDO> {

    default LambdaQueryWrapper<YzSignboardBfDO> buildQueryWrapper(SignboardPageReqVO reqVO) {
        LambdaQueryWrapper<YzSignboardBfDO> wrapper = new LambdaQueryWrapper<YzSignboardBfDO>()
                .like(StrUtil.isNotBlank(reqVO.getSignboardName()),
                        YzSignboardBfDO::getSignboardName, reqVO.getSignboardName())
                .like(StrUtil.isNotBlank(reqVO.getSignboardCode()),
                        YzSignboardBfDO::getSignboardCode, reqVO.getSignboardCode())
                .in(reqVO.getSignboardLevel() != null && !reqVO.getSignboardLevel().isEmpty(),
                        YzSignboardBfDO::getSignboardLevel, reqVO.getSignboardLevel())
                .eq(reqVO.getIsScreenDisplay() != null,
                        YzSignboardBfDO::getIsScreenDisplay, reqVO.getIsScreenDisplay())
                .orderByDesc(YzSignboardBfDO::getUpdateTime, YzSignboardBfDO::getCreateTime);
        ...
        if (StrUtil.isNotBlank(reqVO.getRiverKeyword())) {
            wrapper.and(w -> w.like(YzSignboardBfDO::getRiverChannelName, reqVO.getRiverKeyword())
                    .or()
                    .like(YzSignboardBfDO::getRiverSectionName, reqVO.getRiverKeyword()));
        }
        return wrapper;
    }
    ...
    List<YzSignboardBfDO> selectListByAreaCodes(@Param("areaCodes") String[] areaCodes);
}
