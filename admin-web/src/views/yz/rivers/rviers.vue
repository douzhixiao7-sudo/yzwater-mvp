          ...
          <el-col :span="24">
            ...
            <div v-else>
              ...
              <div class="map-actions">
                ...
                <el-button v-if="!createForm.facilityId" disabled>请先保存河道基础信息</el-button>
              </div>
...
import { getWaterFacilityDetail, updateWaterFacilityGeometry } from '@/api/gis/waterFacility'
...
const openEditDialog = async (id: string, mode: 'edit' | 'view') => {
  ...
  if (detail?.facilityId) {
    try {
      const facility = await getWaterFacilityDetail(String(detail.facilityId))
      facilityGeoJson.value = facility?.geometryGeoJson || ''
    } catch {
      facilityGeoJson.value = ''
    }
  }
...
const handleSaveFacilityGeometry = async () => {
  ...
  try {
    await updateWaterFacilityGeometry(String(createForm.facilityId), {
