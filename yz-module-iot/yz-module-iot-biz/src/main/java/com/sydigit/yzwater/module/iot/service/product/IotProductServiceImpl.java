...
import com.sydigit.yzwater.module.iot.dal.mysql.product.IotProductMapper;
...
public class IotProductServiceImpl implements IotProductService {
    ...
    @Override
    @DSTransactional(rollbackFor = Exception.class)
    @CacheEvict(value = RedisKeyConstants.PRODUCT, allEntries = true)
    public Integer publishAllUnpublished() {
        List<IotProductDO> products = productMapper.selectListByStatus(IotProductStatusEnum.UNPUBLISHED.getStatus());
        ...
        List<IotProductDO> updates = convertList(products,
                product -> IotProductDO.builder()
                        .id(product.getId())
                        .status(IotProductStatusEnum.PUBLISHED.getStatus())
                        .build());
        productMapper.updateBatch(updates);
