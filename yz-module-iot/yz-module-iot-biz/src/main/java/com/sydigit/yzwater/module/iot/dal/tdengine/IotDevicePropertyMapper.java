...
import com.sydigit.yzwater.module.iot.framework.tdengine.core.annotation.TDengineDS;
...
public interface IotDevicePropertyMapper {
    ...
    default void alterProductPropertySTable(Long productId,
        ...
        oldFields.removeIf(field -> StrUtil.equalsAnyIgnoreCase(field.getField(),
                TDengineTableField.FIELD_TS, "report_time", "device_id"));
        List<TDengineTableField> addFields = newFields.stream().filter( // 新增的字段
                        newField -> oldFields.stream().noneMatch(
                                oldField -> StrUtil.equalsIgnoreCase(oldField.getField(), newField.getField())))
                .collect(Collectors.toList());
        List<TDengineTableField> dropFields = oldFields.stream().filter( // 删除的字段
                        oldField -> newFields.stream().noneMatch(
                                n -> StrUtil.equalsIgnoreCase(n.getField(), oldField.getField())))
                .collect(Collectors.toList());
        ...
        newFields.forEach(newField -> {
            TDengineTableField oldField = CollUtil.findOne(oldFields,
                    field -> StrUtil.equalsIgnoreCase(field.getField(), newField.getField()));
            if (oldField == null) {
                return;
            }
    ...
    List<Map<String, Object>> selectColumnsInRange(@Param("deviceId") Long deviceId,
