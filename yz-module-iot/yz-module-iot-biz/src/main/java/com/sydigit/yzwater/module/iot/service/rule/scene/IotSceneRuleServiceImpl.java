package com.sydigit.yzwater.module.iot.service.rule.scene;
...
import com.sydigit.yzwater.module.iot.controller.admin.rule.vo.scene.IotSceneRulePageReqVO;
import com.sydigit.yzwater.module.iot.controller.admin.rule.vo.scene.IotSceneRuleSaveReqVO;
...
import com.sydigit.yzwater.module.iot.dal.mysql.rule.IotSceneRuleMapper;
import com.sydigit.yzwater.module.iot.dal.redis.RedisKeyConstants;
import com.sydigit.yzwater.module.iot.enums.rule.IotSceneRuleTriggerTypeEnum;
...
import com.sydigit.yzwater.module.iot.service.rule.scene.action.IotSceneRuleAction;
import com.sydigit.yzwater.module.iot.service.rule.scene.matcher.IotSceneRuleMatcherManager;
import com.sydigit.yzwater.module.iot.service.rule.scene.timer.IotSceneRuleTimerHandler;
...
import static com.sydigit.yzwater.module.iot.enums.ErrorCodeConstants.RULE_SCENE_NOT_EXISTS;
...
public class IotSceneRuleServiceImpl implements IotSceneRuleService {
    ...
    public void executeSceneRuleByDevice(IotDeviceMessage message) {
        if (message == null || message.getDeviceId() == null) {
            log.warn("[executeSceneRuleByDevice][设备消息为空或设备ID为空，不执行规则场景]");
            return;
        }
        // 1.1 这里的 tenantId，通过设备获取；
        IotDeviceDO device = deviceService.getDeviceFromCache(message.getDeviceId());
        if (device == null) {
            log.warn("[executeSceneRuleByDevice][设备({})不存在，可能已被删除，跳过执行]",
                    message.getDeviceId());
            return;
        }
        ...
        if (device.getTenantId() == null) {
            TenantUtils.executeIgnore(executeTask);
            return;
        }
        TenantUtils.execute(device.getTenantId(), executeTask);
    }
