...
import com.sydigit.yzwater.module.iot.service.device.IotVideoDeviceSyncService;
...
public class IotDeviceController {
    ...
    @Resource
    private IotVideoDeviceSyncService iotVideoDeviceSyncService;

    @PostMapping("/create")
    @Operation(summary = "生成设备二维码（历史设备补齐）")
    ...
    @Parameter(name = "id", description = "设备编号", required = true, example = "1024")
    ...
    @Operation(summary = "为二维码为空的设备批量补齐二维码")
    @PreAuthorize("@ss.hasPermission('iot:device:update')")
    public CommonResult<Integer> generateDeviceQrCode() {
        return success(deviceService.generateDeviceQrCode());
    }
    ...
    @GetMapping("/list")
    @Operation(summary = "获得设备列表（不分页）")
    public CommonResult<List<IotDeviceRespVO>> getDeviceList(@Valid IotDevicePageReqVO reqVO) {
        reqVO.setPageSize(PageParam.PAGE_SIZE_NONE);
        PageResult<IotDeviceDO> pageResult = deviceService.getDevicePage(reqVO);
        return success(BeanUtils.toBean(pageResult.getList(), IotDeviceRespVO.class));
    }
    ...
    public CommonResult<List<IotDeviceRespVO>> getDeviceSimpleList(
        ...
        return success(convertList(list, device ->  // 只返回前端下拉所需字段
                new IotDeviceRespVO().setId(device.getId()).setDeviceName(device.getDeviceName())
                        .setNickname(device.getNickname()).setSerialNumber(device.getSerialNumber())
                        .setStationId(device.getStationId()).setDeviceType(device.getDeviceType())
                        .setEquipmentModel(device.getEquipmentModel())
    ...
    @PostMapping("/sync-video-cameras")
    @Operation(summary = "同步视频摄像头到 IoT 设备")
    @PreAuthorize("@ss.hasPermission('iot:device:update')")
    public CommonResult<IotVideoDeviceSyncRespVO> syncVideoCamerasToDevice() {
        return success(iotVideoDeviceSyncService.syncVideoDevices());
    }
