...
public class ReservoirExcelImportService {
    ...
    public ReservoirImportRespVO importExcel(MultipartFile file) {
        ...
        Map<String, List<YzWaterReservoirDO>> reservoirByName = loadReservoirByName(reservoirNames);
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            try {
                ...
                List<YzWaterReservoirDO> matchedReservoirs = reservoirByName.getOrDefault(reservoirName, List.of());
                ...
                int rowUpdated = 0;
                for (YzWaterReservoirDO reservoir : matchedReservoirs) {
    ...
    private Map<String, List<YzWaterReservoirDO>> loadReservoirByName(Set<String> reservoirNames) {
        if (CollUtil.isEmpty(reservoirNames)) {
            return Map.of();
        }
        List<YzWaterReservoirDO> reservoirs = reservoirMapper.selectList(new LambdaQueryWrapper<YzWaterReservoirDO>()
