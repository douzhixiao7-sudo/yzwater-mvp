<template>
    <div class="timeline-list">
            ...
            <div class="timeline-content">
                ...
                <div class="fileList" v-if="verificationTask?.verifyHandleImages && verificationTask.verifyHandleImages.length > 0">
                    <div class="fileItem" v-for="(fileUrl, index) in verificationTask.verifyHandleImages" :key="index"
                        @click="previewFile(fileUrl)">
                        <img v-if="isImage(fileUrl)" :src="resolveFileUrl(fileUrl)" alt="核验图片" />
                        <div v-else class="video-icon">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                                <path d="M8 5v14l11-7z" fill="#fff" />
                            </svg>
        ...
        <div class="timeline-item">
            ...
            <div class="timeline-content">
                <div class="timeline-header">
                    <span class="timeline-status">已驳回</span>
                </div>
            </div>
        </div>
    </div>
            ...
            try {
                ...
                if (response && response.data) {
                    ...
                    if (this.feedbackDetail.statusTasks && Array.isArray(this.feedbackDetail.statusTasks)) {
                        // 查找 status === 1 的任务（已驳回状态的任务，可能包含已受理、已处理、已核验的数据）
                        const rejectedTask = this.feedbackDetail.statusTasks.find(task => task.status === 1);
                        if (rejectedTask) {
                            // 已受理节点：使用 status === 1 任务中的审核相关字段
                            this.currentTask = rejectedTask;
                            // 已处理节点：使用 status === 1 任务中的处理相关字段
                            this.processingTask = rejectedTask;
                            // 已核验节点：使用 status === 1 任务中的核验相关字段
                            this.verificationTask = rejectedTask;
                        } else {
                            // 如果没有 status === 1 的任务，尝试查找其他状态的任务
                ...
                console.error('RejectedStatus - 获取反馈详情失败:', error);
            }
        },
